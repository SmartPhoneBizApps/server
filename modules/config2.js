...
module.exports = {
  ...
  getApplication: function (app) {
    const myApp = App.findOne({
      applicationID: app,
    });
    return myApp;
  },
  getRole: function (role) {
    const myRole = Role.findOne({
      role: role,
    });
    return myRole;
  },
  ...
  findAndUpdateItem: function (mdata, app) {
    ...
    for (let index = 0; index < mdata.length; index++) {
      result2 = Model.findOneAndUpdate(
        {
          ID: mdata.ID,
          ItemNumber: mdata[index]["ItemNumber"],
        },
        mdata[index],
        {
          new: true,
