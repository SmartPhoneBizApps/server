...
function createVariables(v1, count, val) {
  return (v1[count] = val);
}
con = 0;
con1 = 0;
    ...
    if (cardConfig.hasOwnProperty(k1)) {
      for (const k2 in cardConfig.Tabs) {
        ...
        if (
          ...
          con1 = con1 + 1;
          tabCX["tab" + con1] = { ...cardConfig.Tabs[k2] };
          // data for the requested tab is found in the Card Configuration
          //const tabCX["tab"+con1] = { ...cardConfig.Tabs[k2] };

          // Now loop through Tiles... in Card Configuration
          for (const key3 in tabCX["tab" + con1].Tiles) {
            // console.log(tabCX["tab"+con1].Tiles[key3].CardID);
            if (tabCX["tab" + con1].Tiles.hasOwnProperty(key3)) {
              // Card / Tile found
              const configCardID = tabCX["tab" + con1].Tiles[key3].CardID;
              // Filtering the userSettings data based on application ID
              myData =
                userSetCards[role][
                  tabCX["tab" + con1].Tiles[key3].applicationID
                ];
                ...
                if (myData.hasOwnProperty(key4) & (myData[key4] == true)) {
                  if (key4 == configCardID) {
                    ...
                    let q1 = JSON.stringify(
                      tabCX["tab" + con1].Tiles[key3].filters
                    );
                    let query;
                    //let results;
                    ...
                    if (applicationID == "MYDATA") {
                      ...
                      // Select Fields
                      const fList = [];
                      if (tabCX["tab" + con1].Tiles[key3].fieldList) {
                        const fields =
                          tabCX["tab" + con1].Tiles[key3].fieldList;
                        query = query.select(fields);
                        const fList = tabCX["tab" + con1].Tiles[
                          key3
                        ].fieldList.split(" ");
                        //  console.log(fList);
                      }
                      //Sorting
                      if (tabCX["tab" + con1].Tiles[key3].sortBy) {
                        const sortBy = tabCX["tab" + con1].Tiles[key3].sortBy;
                        query = query.sort(sortBy);
                      } else {
                        query = query.sort("-createdAt");
                      }
                      // Check Limits
                      if (tabCX["tab" + con1].Tiles[key3].limit) {
                        const limit =
                          parseInt(tabCX["tab" + con1].Tiles[key3].limit, 10) ||
                          5;
                        const startIndex = 0;
                        query = query.skip(startIndex).limit(limit);
                      }
                      // Populate connected fields
                      if (tabCX["tab" + con1].Tiles[key3].populate) {
                        query = query.populate(populate);
                      }
                      // Get data
                      let results = await query;
                      jsonArray = results;
                      // console.log("MyResults:", jsonArray);
                    }

                    // Read Card Template
                    let fileName3 =
                      "../../cards/templates/" +
                      tabCX["tab" + con1].Tiles[key3].Type +
                      "_template.json";
                    ...
                    console.log("my Key:", key["con" + con]);
                    //var key["con" + con] = require(fileName3);
                    //console.log(key["con" + con]);
                    ...
                    key["con" + con]["sap.card"]["header"] =
                      tabCX["tab" + con1].Tiles[key3].header;

                    //-------------------------------------------------
                    //object1 : set group
                    if (tabCX["tab" + con1].Tiles[key3].Type == "object1") {
                      k = 0;
                      tabCX["tab" + con1].Tiles[key3].groups.forEach((gr1) => {
                        key["con" + con]["sap.card"]["content"]["groups"][
                          key3
                        ] = {
                          ...gr1,
                        };
                    ...
                    //-------------------------------------------------
                    //table1
                    if (tabCX["tab" + con1].Tiles[key3].Type == "table1") {
                      ///// Set Coloumns ////////
                      k = 0;
                      tabCX["tab" + con1].Tiles[key3].columns.forEach(
                        (col1) => {
                          key["con" + con]["sap.card"]["content"]["row"][
                            "columns"
                          ][k] = col1;
                          k = k + 1;
                          console.log("Console Table : ", col1);
                        }
                      );
                      /// Add JSON Data
                      myD["json"] = jsonArray;
                      //cardTemp["sap.card"]["data"] = { ...myD };
                      key["con" + con]["sap.card"]["data"] = Object.assign(
                        {},
                        myD
                      );
                    ...
                    if (tabCX["tab" + con1].Tiles[key3].Type == "object1") {
                      k = 0;
                      tabCX["tab" + con1].Tiles[key3].actions.forEach((el1) => {
                    ...
                    //list2
                    if (tabCX["tab" + con1].Tiles[key3].Type == "list2") {
                      k = 0;
                      tabCX["tab" + con1].Tiles[key3].actions.forEach((el1) => {
                    ...
                    if (
                      tabCX["tab" + con1].Tiles[key3].Type == "list1" ||
                      tabCX["tab" + con1].Tiles[key3].Type == "list2" ||
                      tabCX["tab" + con1].Tiles[key3].Type == "timeline1" ||
                      tabCX["tab" + con1].Tiles[key3].Type == "quicklinks1"
                    ) {
                      /// Add Data
                      myD["json"] = { ...jsonArray };
                    ...
                    counter = counter + 1;
                    let TileID =
                      tabCX["tab" + con1].Tiles[key3].Type + "_" + counter;
                    myTiles[TileID] = { ...key["con" + con] };
                    console.log(key);
                    key["con" + con] = {};
